...
rdd = sc.____(range(1, 1000001), ____)
...
# 4. map+filter 연산
map_filter_result, t1 = measure_time(lambda: rdd.____(lambda x: x % 2 == 0).____(lambda x: x * 2).____())
print("[map + filter] 개수:", len(map_filter_result))
print("[map + filter] 샘플:", map_filter_result[:5])
print("[map + filter] 시간:", round(t1, 4), "초")

# 5. flatMap 연산
flatmap_result, t2 = measure_time(lambda: rdd.____(lambda x: [x * 2] if x % 2 == 0 else []).____())
...
mappart_result, t3 = measure_time(lambda: rdd.____(transform_partition).____())
