...
BUS_API_KEY = "486878414f67786835304448424d56"  # 서울시 버스 교통 데이터 API 키 (빈칸 채우기)
SUBWAY_API_KEY = "7a756e724e6778683636475957425a"  # 서울시 지하철 교통 데이터 API 키 (빈칸 채우기)
...
def get_subway_data(date):
    ...
    for row in root.findall('.//row'):
        subway_data.append({
            ...
            '하차인원': int(row.find('GTOF_TNOPE').text) if row.find('GTOF_TNOPE') is not None else ''  # row에서 'GTOFF_TNOPE' 태그의 텍스트 추출 후 정수로 변환)
        })
