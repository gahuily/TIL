...
def get_movie_recommendation(rating):
    if (rating >= 9):
        print('Recommendataion: Inception')
    elif (rating >= 8) & (rating < 9):
        print('Recommendation: Interstellar')
    elif (rating < 8) & (rating >= 7):
        print('Recommendation: Dunkirk')
    else:
        print('Tenet')
    return

get_movie_recommendation(float(input()))