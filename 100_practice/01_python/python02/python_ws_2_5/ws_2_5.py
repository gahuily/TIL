# 영화관 시스템을 확장하여 상속을 활용한 클래스를 정의하시오. 
# 부모 클래스와 자식 클래스의 개념을 활용하여 영화관의 기본 정보를 설정하고, 
# 영화관의 상태를 관리하는 메서드를 구현하시오.
...
# 'reserve_seat' 메서드는 예약 가능한 좌석이 있는 경우,
# 'reserved_seats'를 1 증가시키고 예약 성공 메시지를 반환한다.
...
class MovieTheater(Theater):
    def __init__(self, name, total_seats, reserved_seats, total_movies):
        super().__init__(name, total_seats, reserved_seats)
        self.total_movies = total_movies

    def add_movie(self):
        self.total_movies += 1
        return '영화를 성공적으로 추가하였습니다.'
...
mega = MovieTheater('메가박스', 100, 0, 0)
