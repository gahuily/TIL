...
# VIPMovieTheater 클래스는 VIP 좌석을 예약하는 reserve_vip_seat 메서드를 가진다.
    # reserve_vip_seat 메서드는 예약 가능한 VIP 좌석이 있는 경우,
    # vip_seats를 1 감소시키고 예약 성공 메시지를 반환한다.
    # 예약 가능한 VIP 좌석이 없는 경우, 예약 실패 메시지를 반환한다.

# VIPMovieTheater 클래스는 reserve_seat 메서드를 오버라이딩하여, VIP 좌석이 먼저 예약되도록 한다.
    # VIP 좌석이 예약 가능한 경우,
    # reserve_vip_seat 메서드를 호출하여 VIP 좌석을 예약한다.
    # VIP 좌석이 예약 불가능한 경우,
    # 부모 클래스의 reserve_seat 메서드를 호출하여 일반 좌석을 예약한다.
...
k = VIPMovieTheater('메가박스', 100, 3)

print(k.reserve_seat())
print(k.reserve_seat())
print(k.reserve_seat())
print(k.reserve_seat())
print(k.reserve_vip_seat())